...
const backHandler=()=>{
  dispatch({type:"toggle-post-opener",value:false}) 
...
const getPostData=async()=>{
  const docRef=doc(db,"posts",postId)
    ...
    setRank(docSnap.data())
}


const {sendRequest,isLoading,error}=useHttp({url:`https://react-60170-default-rtdb.firebaseio.com/commentData/${postId}.json`},transformedRankData)
...
getPostData()
  ...
  return(
    ...
    <header className={classes['post-header']}>
      ...
      <img className={classes['post-img']} src={getPost.img} height={80} width={80}/>
      <div className={classes['posted-details']}>
        <h3 className={classes['post-name']}>{getPost.name}</h3>
      </div>
    ...
    <section>
      <div className={classes['post-question']}>
        <p>{getPost.postDetails}</p>
      </div>
    </section>
    <section>
      <Discussion refresh={refresh} id={getPost.id} />
      <AddComment refresh={refreshHandler} id={getPost.id} />
    </section>
    </div>
  )
}
